  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6 md:p-8 flex flex-col min-h-[420px] shadow-lg shadow-black/20">
                 ...
                 return (
                   <div key={task.id} onClick={() => toggleTaskStatus(task.id, 'done')} className="flex items-center gap-4 p-4 rounded-xl border border-slate-800/80 hover:border-blue-500/50 bg-slate-900/40 cursor-pointer group transition-all hover:bg-slate-800/50 hover:shadow-md">
                     ...
                     <div className="flex-1 min-w-0 flex justify-between items-center">
                       <div>
                         <p className="text-sm font-bold text-slate-200 group-hover:text-blue-100 transition-colors truncate">{task.title}</p>
        ...
        <Card className="p-6 md:p-8 flex flex-col gap-4 min-h-[420px] shadow-lg shadow-black/20">
           <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-2"><Layout size={14}/> Tiến độ Dự án Hiện tại</h3>
           <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
             {projects.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-500 py-10">
                   <Briefcase size={48} className="mb-4 opacity-30" />
                   <p className="text-sm font-bold text-slate-400">Chưa có dự án nào.</p>
                </div>
             ) : (
               projects.map(p => {
                  ...
                  return (
                    <div key={p.id} className="bg-slate-900/40 p-4 rounded-xl border border-slate-800/50">
                      <div className="flex justify-between items-center mb-3">
                        <div>
                          <p className="text-sm font-bold text-slate-200 truncate pr-2">{p.name}</p>
                          <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-wider font-mono">{formatDate(p.startDate)} - {formatDate(p.endDate)}</p>
                        </div>
                        <span className={`text-sm font-black ${pProg === 100 ? 'text-emerald-400' : 'text-blue-400'}`}>{pProg}%</span>
                      </div>
                      <ProgressBar progress={pProg} colorClass={`bg-${p.color}-500`} heightClass="h-2" />
                    </div>
                  )
               })
             )}
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500">
      <div className="flex justify-between items-end mb-6 shrink-0 no-print">
        ...
        <button onClick={() => window.print()} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg flex items-center gap-2 active:scale-95">
          <Printer size={18} /> In / Lưu PDF
        </button>
      </div>

      <div className="flex-1 flex flex-col md:flex-row gap-8 min-h-0">
        <div className="w-full md:w-64 shrink-0 bg-slate-900 border border-slate-800 rounded-2xl p-5 h-max no-print shadow-lg">
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" checked={config.showTimeline} onChange={e => setConfig({...config, showTimeline: e.target.checked})} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-blue-600 focus:ring-blue-600 focus:ring-offset-slate-900 cursor-pointer" />
              <span className="text-sm font-medium text-slate-300 group-hover:text-slate-50 transition-colors">Lịch trình công việc</span>
            </label>
            <label className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" checked={config.showProjects} onChange={e => setConfig({...config, showProjects: e.target.checked})} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-blue-600 focus:ring-blue-600 focus:ring-offset-slate-900 cursor-pointer" />
              <span className="text-sm font-medium text-slate-300 group-hover:text-slate-50 transition-colors">Tiến độ Dự án</span>
            </label>
            <label className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" checked={config.showCRM} onChange={e => setConfig({...config, showCRM: e.target.checked})} className="w-4 h-4 rounded bg-slate-800 border-slate-700 text-blue-600 focus:ring-blue-600 focus:ring-offset-slate-900 cursor-pointer" />
          ...
          <div className="space-y-3">
            <div>
              ...
              <input type="date" value={dateRange.start} onChange={e => setDateRange({...dateRange, start: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-sm text-slate-300 outline-none [color-scheme:dark] focus:border-blue-500 transition-colors cursor-pointer" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Đến ngày</label>
              <input type="date" value={dateRange.end} onChange={e => setDateRange({...dateRange, end: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-sm text-slate-300 outline-none [color-scheme:dark] focus:border-blue-500 transition-colors cursor-pointer" />
            </div>
            {(dateRange.start || dateRange.end) && (
              <button onClick={() => setDateRange({start: '', end: ''})} className="w-full mt-2 text-xs text-rose-400 hover:text-rose-300 font-medium text-right transition-colors p-2 active:bg-slate-800 rounded">Xóa bộ lọc</button>
            )}
          </div>
        </div>
          ...
          <Card className="p-6">
            ...
            {user.level === 'Founder' ? (
              <form onSubmit={generateCode} className="space-y-4">
                ...
                <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-600/20 active:scale-95">
                  Tạo Mã Truy Cập
                </button>
              </form>
            ) : (
                      ...
                      {m.status === 'active' ? (
                        <div className="flex items-center justify-end gap-3">
                           ...
                           {user.level === 'Founder' && m.role !== 'Founder' && (
                             <button onClick={() => revokeCode(m.id, m.name)} className="text-xs text-rose-400 hover:underline font-medium p-2">Thu hồi</button>
                           )}
                        </div>
                      ) : (
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-6 shrink-0 z-10">
        ...
        <div className="flex flex-wrap items-center gap-3">
           <div className="bg-slate-900 border border-slate-800 rounded-xl p-1 flex shadow-sm">
              <button onClick={() => setViewMode('calendar')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all active:scale-95 ${viewMode === 'calendar' ? 'bg-slate-800 text-slate-50 shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Lịch trình Block</button>
              <button onClick={() => setViewMode('details')} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all active:scale-95 ${viewMode === 'details' ? 'bg-slate-800 text-slate-50 shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Chi tiết Gantt</button>
           </div>
           <button onClick={handleOpenCreate} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2.5 rounded-xl text-sm font-bold transition-colors shadow-lg shadow-blue-600/20 flex items-center gap-2 active:scale-95">
          ...
          <Card className="p-6 md:p-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pb-6 border-b border-slate-800 gap-4">
                 ...
                 <div className="text-xs font-mono text-slate-400 flex items-center gap-4">
                    ...
                    <div className="flex gap-1 bg-slate-900 rounded-lg p-1 border border-slate-800">
                       <button onClick={prevMonth} className="p-2 hover:bg-slate-800 rounded text-slate-500 hover:text-slate-300 transition-colors active:scale-90"><ChevronLeft size={16}/></button>
                       <button onClick={() => setCurrentCalMonth(new Date(today.getFullYear(), today.getMonth(), 1))} className="px-3 font-bold hover:bg-slate-800 rounded text-slate-500 hover:text-slate-300 transition-colors active:scale-95">Nay</button>
                       <button onClick={nextMonth} className="p-2 hover:bg-slate-800 rounded text-slate-500 hover:text-slate-300 transition-colors active:scale-90"><ChevronRight size={16}/></button>
                    </div>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-7 gap-3 md:gap-4">
                 ...
                 {calendarDays.map((date, idx) => {
                     ...
                     return (
                         <div key={idx} onClick={() => setSelectedDayInfo(date)} className={`min-h-[90px] md:min-h-[110px] rounded-2xl p-3 flex flex-col justify-between transition-all group cursor-pointer active:scale-95 ${dayStatus === 'available' ? 'bg-[#0f172a]/60 border border-slate-800/80 hover:border-slate-600 hover:bg-[#1e293b]' : 'bg-slate-800/50 border border-slate-700 shadow-sm hover:border-blue-500/50 hover:bg-slate-800'}`}>
                           ...
                           <div className="min-w-0 flex-1">
                              <div className="flex items-center justify-between">
                                  ...
                                  <div className="hidden group-hover:flex items-center gap-1 shrink-0 ml-2">
                                    <button onClick={(e) => handleOpenEdit(project, e)} className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-white hover:bg-blue-600 transition-colors"><Edit2 size={12}/></button>
                                    <button onClick={(e) => handleOpenDelete(project, e)} className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-white hover:bg-rose-600 transition-colors"><Trash size={12}/></button>
                                  </div>
                              </div>
                           </div>
                         </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-slate-900 rounded-3xl w-full max-w-lg border border-slate-800 shadow-2xl p-6 md:p-8 flex flex-col max-h-[85vh]">
            <div className="flex justify-between items-center mb-6 shrink-0">
              ...
              <button onClick={() => setSelectedDayInfo(null)} className="text-slate-500 hover:text-slate-50 bg-slate-800 hover:bg-slate-700 p-2 rounded-full transition-colors active:scale-90"><X size={18}/></button>
            </div>
            ...
            <div className="pt-4 mt-4 border-t border-slate-800 flex justify-end shrink-0">
               <button onClick={() => setSelectedDayInfo(null)} className="px-5 py-2.5 text-sm font-bold text-white bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors active:scale-95">Đóng cửa sổ</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-slate-900 rounded-3xl w-full max-w-md border border-slate-800 shadow-2xl p-8">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-slate-500 hover:text-slate-50 bg-slate-800 hover:bg-slate-700 p-2 rounded-full transition-colors active:scale-90"><X size={18}/></button>
            </div>
            <form onSubmit={handleSaveProject} className="space-y-5">
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-slate-800 mt-6">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-sm font-bold text-slate-400 hover:text-slate-50 bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors active:scale-95">Hủy</button>
                <button type="submit" className="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-500 rounded-xl shadow-lg shadow-blue-600/20 transition-all active:scale-95">{editingProject ? 'Cập nhật' : 'Tạo Dự Án'}</button>
              </div>
            </form>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-6 shrink-0">
        ...
        <button onClick={() => openModal()} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-600/20 active:scale-95 flex items-center justify-center gap-2">
          <Plus size={18} /> Thêm Task
        </button>
      </div>

      <div className="flex gap-6 flex-1 min-h-0 overflow-x-auto pb-4 custom-scrollbar snap-x touch-pan-x">
        {columns.map(col => (
          <div key={col.id} onDragOver={onDragOver} onDrop={(e) => onDrop(e, col.id)} className="flex-1 min-w-[300px] max-w-[400px] bg-slate-900/40 rounded-2xl border border-slate-800 flex flex-col snap-center">
            ...
            <div className="p-4 space-y-3 flex-1 overflow-y-auto custom-scrollbar">
              {filteredTasks.filter(t => t.status === col.id).map(task => {
                ...
                const pColor = task.priority === 'high' ? 'rose' : task.priority === 'medium' ? 'amber' : 'slate';
                return (
                  <div key={task.id} draggable onDragStart={(e) => onDragStart(e, task)} onClick={() => openModal(task)} className={`bg-slate-800 border border-slate-700 p-4 rounded-xl cursor-grab active:cursor-grabbing hover:border-blue-500/50 transition-all shadow-sm group select-none ${col.id === 'done' ? 'opacity-50' : ''}`}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-3xl w-full max-w-lg border border-slate-800 p-6 md:p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-slate-50">{editingTask ? 'Chỉnh sửa Task' : 'Tạo Task mới'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-500 hover:text-slate-50 bg-slate-800 p-2 rounded-full active:scale-90"><X size={18}/></button>
            </div>
            <form onSubmit={handleSaveTask} className="space-y-5">
              ...
              <div className="flex justify-between items-center pt-6 border-t border-slate-800 mt-6">
                {editingTask ? (
                  <button type="button" onClick={() => handleDeleteTask(editingTask)} className="text-rose-400 hover:text-slate-50 bg-rose-500/10 hover:bg-rose-600 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors active:scale-95">
                    <Trash2 size={16} /> Xóa
                  </button>
                ) : <div/>}
                <div className="flex gap-3">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-sm font-bold text-slate-400 bg-slate-800 hover:bg-slate-700 rounded-xl active:scale-95">Hủy</button>
                  <button type="submit" className="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-500 rounded-xl shadow-lg active:scale-95">Lưu lại</button>
                </div>
              </div>
            </form>
          </div>
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 mb-6 shrink-0">
        ...
        <button onClick={() => openModal()} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg flex items-center justify-center gap-2 active:scale-95"><Plus size={18} /> Thêm Khách hàng</button>
      </div>
            ...
            <tbody className="divide-y divide-slate-800">
              {filteredClients.map(client => (
                <tr key={client.id} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-4 text-right"><button onClick={() => openModal(client)} className="p-3 text-slate-500 hover:text-blue-400 bg-slate-900 rounded-xl border border-slate-800 active:scale-90"><Edit2 size={16}/></button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-3xl max-w-md w-full border border-slate-800 p-6 md:p-8">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex justify-between items-center pt-6 mt-6 border-t border-slate-800">
                {editingClient ? <button type="button" onClick={() => deleteClient(editingClient.id)} className="text-rose-400 hover:underline text-sm font-bold p-2">Xóa</button> : <div/>}
                <div className="flex gap-3"><button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 bg-slate-800 rounded-xl text-sm font-bold active:scale-95">Hủy</button><button type="submit" className="px-5 py-2.5 bg-blue-600 rounded-xl text-sm font-bold text-white active:scale-95">Lưu</button></div>
              </div>
            </form>
          </div>
        </div>
...
export default function App() {
  ...
  const NavItem = ({ icon: Icon, label, id }) => (
    <button onClick={() => { setActiveTab(id); setSearchQuery(''); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3.5 md:py-3 rounded-xl transition-all text-sm font-medium active:scale-95 ${activeTab === id ? 'bg-blue-600/10 text-blue-400 font-bold border border-blue-500/20' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200 border border-transparent'}`}>
      <Icon size={20} className={activeTab === id ? 'text-blue-400' : 'opacity-70 md:w-[18px] md:h-[18px]'} />
      {label}
    </button>
  );

  return (
    <div className="flex h-[100dvh] bg-slate-950 text-slate-50 font-sans overflow-hidden selection:bg-blue-500/30 app-container">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 w-72 bg-slate-900 border-r border-slate-800 flex flex-col z-50 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} no-print`}>
        <div className="h-20 flex items-center px-6 border-b border-slate-800 shrink-0">
          ...
          <button className="ml-auto lg:hidden p-2 text-slate-400" onClick={() => setIsSidebarOpen(false)}>
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900 shrink-0">
          <div className="flex items-center justify-between px-4 py-3 bg-slate-800 hover:bg-slate-700 transition-colors rounded-2xl border border-slate-700 cursor-pointer group shadow-sm active:scale-95">
            <div className="flex items-center gap-3 min-w-0" onClick={() => {setActiveTab('team'); setIsSidebarOpen(false);}}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold shrink-0 shadow-inner text-white border border-white/10 ${user.level === 'Founder' ? 'bg-violet-600' : 'bg-blue-600'}`}>
            ...
            <button onClick={(e) => { e.stopPropagation(); handleLogoutRequest(); }} className="text-slate-500 hover:text-rose-400 transition-colors p-2" title="Đăng xuất"><LogOut size={18} /></button>
          </div>
        </div>
      </aside>

      <main className="flex-1 flex flex-col h-[100dvh] overflow-hidden bg-slate-950 relative print-area">
        <header className="h-16 md:h-20 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-4 md:px-8 shrink-0 z-10 sticky top-0 no-print">
             <div className="flex items-center gap-3 w-full max-w-md">
                <button className="lg:hidden p-2 text-slate-400 active:scale-90" onClick={() => setIsSidebarOpen(true)}>
                  <Layout size={24} />
                </button>
                <div className="relative w-full">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
                  <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Tìm kiếm nhanh..." className="w-full bg-slate-950 border border-slate-800 rounded-full py-2.5 pl-11 pr-4 text-sm text-slate-50 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors shadow-inner placeholder:text-slate-500" />
                </div>
             </div>
             <div className="flex items-center gap-3 md:gap-5 ml-auto pl-2">
               <div className="hidden sm:block"><FocusTimer /></div>
               <button className="text-slate-500 hover:text-slate-50 transition-colors relative p-2 active:scale-90" title="Thông báo">
                 <Bell size={20}/>
               </button>
             </div>
        </header>

        <div className="flex-1 overflow-y-auto p-4 md:p-10 custom-scrollbar relative no-print-padding">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .app-container *:not(input):not(textarea) {
          user-select: none;
        }
        input, textarea {
          user-select: text;
